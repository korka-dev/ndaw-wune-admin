...
export default function EvaluateursPage() {
  ...
  const load = (isFirstLoad = false) => {
    ...
    Promise.all([
      ...
      .then(([rEvs, rSchools, rTeachers]) => {
        setEvs(rEvs.data.items ?? []);
  ...
  const doDelete = async (ev: Evaluateur) => {
    setLoading(true);
    try { await evaluateursApi.delete(ev.id); load(false); setModal(null); }
    finally { setLoading(false); }
  };

  const doToggle = async (ev: Evaluateur) => {
    setLoading(true);
    try { await evaluateursApi.toggleStatus(ev.id); load(false); setModal(null); }
    finally { setLoading(false); }
  };
      ...
      <div className="bg-surface rounded-2xl border border-border overflow-hidden flex-1">
        <table className="w-full text-sm table-fixed">
          ...
          <tbody>
            ...
            {!dataLoading && !dataError && paginated.map(ev => {
